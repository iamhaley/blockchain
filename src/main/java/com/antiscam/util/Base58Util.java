...
/**
 * copy from Google bitcoingj implement
 * AddressFormatException to RuntimeException
 * Sha256Hash to EncryptUtil
 */
public class Base58Util {
    ...
    public static String encodeChecked(int version, byte[] payload) {
        ...
        // 1 byte version + data bytes
        byte[] addressBytes = new byte[1 + payload.length];
        addressBytes[0] = (byte) version;
        System.arraycopy(payload, 0, addressBytes, 1, payload.length);
        byte[] checksum = EncryptUtil.hashTwice(addressBytes, Algorithm.SHA256, 1, payload.length);
        //  + 4 bytes check code (a truncated hash)
        byte[] addressByteWithChecksum = new byte[addressBytes.length + 4];
        System.arraycopy(addressBytes, 0, addressByteWithChecksum, 0, addressBytes.length);
        System.arraycopy(checksum, 0, addressByteWithChecksum, payload.length + 1, 4);
        return Base58Util.encode(addressByteWithChecksum);
    }
    ...
    public static byte[] decodeChecked(String input) throws RuntimeException {
        ...
        byte[] actualChecksum = Arrays.copyOfRange(EncryptUtil.hashTwice(data, Algorithm.SHA256, 1, data.length - 1), 0, 4);
